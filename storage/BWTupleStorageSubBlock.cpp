...
namespace quickstep {
/*
BWTupleStorageSubBlock::BWTupleStorageSubBlock(const TupleStorageSubBlock &tuple_store,
...
TupleIdSequence* BWTupleStorageSubBlock::getMatchesForPredicate(const ComparisonPredicate &predicate,
 ...
 if (!predicate.isAttributeLiteralComparisonPredicate()) {
  ...
  //  how to set work on getID column????

  attribute_id key_id = comparison_attribute->getID();
  ...
  const CompressionDictionary * fake_dictionary;
  ...
  switch (result.type) {
    case PredicateTransformResultType::kAll:
      // All non-null tuples.
        sequence = new TupleIdSequence(getMaxTupleID() + 1);
        if (exist_filter) {
